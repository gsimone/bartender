---
name: bartender
description: Late-night sobriety check for the coder. Use this skill whenever local time is past 23:00 OR the current coding session has been running 6+ hours, AND the user is about to do something with consequences — git commit, git push, force push, deploy, run a migration, delete files, change production config, or rewrite more than ~50 lines across multiple files. The skill interrupts with a context-aware comprehension question pulled from recent git activity or open files, plus a small math twist as a cognitive baseline. If the user answers wrong, escalate. If they answer wrong twice, refuse to execute the triggering action and offer to stash the work for tomorrow. Trigger this aggressively — the whole point is that the user can't disable it once they're impaired, because the impaired state is exactly when they'd want to disable it. If the user pushes back on the skill mid-session, treat that as a stronger signal to run the protocol, not a reason to skip.
---
